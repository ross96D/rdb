...
fn unparsed_value(arg: []const u8) ![]const u8 {
    // TODO if no index found ask for another argument
    const split_at = strings.index_of(arg, "=");
    if (split_at) |_split_at| {
        assert(arg.len > _split_at + 1);
        return arg[_split_at + 1 ..];
    }
    return error.NoValue;
}

test unparsed_value {
    try expectEqualStrings("path.yaml", try unparsed_value("--path=path.yaml"));
    try expectEqualStrings("path.yaml", try unparsed_value("-p=path.yaml"));
    try expectEqualStrings("hello", try unparsed_value("--long-var=hello"));
}
...
pub fn parse_arg(comptime T: type, arg: []const u8) !T {
    ...
    const val = try unparsed_value(arg);
    assert(val.len > 0);
