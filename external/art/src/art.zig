...
pub fn Art(comptime T: type) type {
    return struct {
        ...
        pub fn print(t: *Tree) !void {
            ...
            _ = try t.iter(stderr, showCb(@TypeOf(stderr)));
        }
        pub fn printToStream(t: *Tree, stream: anytype) !void {
            _ = try t.iter(stream, showCb(@TypeOf(stream)));
        }
        pub fn displayNode(stream: anytype, _n: ?*Node, depth: usize) void {
            const n = _n orelse return;
            _ = showCb(@TypeOf(stream))(n, stream, depth) catch unreachable;
        }
        ...
        pub fn iter(
            t: *Tree,
            data: anytype,
            callback: fn (*Node, @TypeOf(data), usize) anyerror!bool,
        ) anyerror!bool {
            return t.recursiveIter(t.root, data, 0, callback);
        }
        ...
        pub fn iterAll(
            ...
            return t.recursiveIterAll(t.root, data, 0, callback);
        }
                ...
                if (depth == prefix.len) {
                    if (minimum(n)) |l| {
                        if (leafPrefixMatches(l.*, prefix))
                            return t.recursiveIter(n, data, depth, callback);
                    }
                    return false;
                }
        ...
        fn recursiveInsert(t: *Tree, _n: ?*Node, ref: *?*Node, key: []const u8, value: T, depth: u32) Error!Result {
            ...
            if (n.* == .leaf) {
                ...
                const l2 = try t.makeLeaf(key, value);
            ...
            if (base.partial_len != 0) {
                ...
                if (base.partial_len <= max_prefix_len) {
                    ...
                    mem.copyForwards(u8, &base.partial, base.partial[prefix_diff + 1 ..][0..@min(max_prefix_len, base.partial_len)]);
                } else {
                    base.partial_len -= (prefix_diff + 1);
                    var l = minimum(n) orelse return error.NoMinimum;
                    try t.addChild8(new_node, ref, l.key[depth + prefix_diff], n);
                    mem.copyForwards(u8, &base.partial, l.key[depth + prefix_diff + 1 ..][0..@min(max_prefix_len, base.partial_len)]);
                }

                // Insert the new leaf
                const l = try t.makeLeaf(key, value);
        ...
        fn addChild8(t: *Tree, n: *Node, ref: *?*Node, c: u8, child: *Node) !void {
            if (n.node8.num_children < 8) {
                ...
                mem.copyForwards(?*Node, new_node.node16.children, n.node8.children);
        ...
        fn addChild16(t: *Tree, n: *Node, ref: *?*Node, c: u8, child: anytype) Error!void {
            if (n.node16.num_children < 16) {
                ...
                mem.copyForwards(?*Node, new_node.node48.children, n.node16.children);
                const base = n.baseNode();
                var i: u8 = 0;
                while (i < base.num_children) : (i += 1)
        ...
        /// calls callback in order on leaf nodes until cb returns true
        fn recursiveIter(
            t: *Tree,
            _n: ?*Node,
            data: anytype,
            ...
            const n = _n orelse return error.Missing;
            switch (n.*) {
                .leaf => return callback(n, data, depth),
                .node8, .node16, .node48, .node256 => {
                    var ci = n.childIterator();
                    while (ci.next()) |child| {
                        if (try t.recursiveIter(child, data, depth + 1, callback))
                            return true;
                    }
                },
            }
        ...
        /// calls callback in order on all nodes (not just leaves) until cb returns true
        fn recursiveIterAll(
            t: *Tree,
            _n: ?*Node,
            data: anytype,
            ...
            switch (n.*) {
                ...
                .leaf => return callback(n, data, depth),
                .node8, .node16, .node48, .node256 => {
                    if (callback(n, data, depth)) return true;
                    var ci = n.?.childIterator();
                    while (ci.next()) |child| {
                        if (t.recursiveIterAll(child, data, depth + 1, callback))
                            return true;
                    }
                },
            }
        ...
        pub fn showCb(Type: type) fn (*Node, Type, usize) anyerror!bool {
            return struct {
                pub fn showCb(_n: ?*Node, data: Type, depth: usize) !bool {
                    const streamPrint = struct {
                        fn f(stream: anytype, comptime fmt: []const u8, args: anytype) void {
        ...
        fn recursiveDelete(t: *Tree, _n: ?*Node, ref: *?*Node, key: []const u8, _depth: usize) Error!Result {
            ...
            const child = (opt_child orelse return .missing).*;
            if (child.* == .leaf) {
                const l = child.*.leaf;
                if (mem.eql(u8, l.key, key)) {
        ...
        fn removeChild8(t: *Tree, _n: ?*Node, ref: *?*Node, l: ?**Node) Error!void {
            ...
            const base = n.baseNode();
            mem.copyForwards(u8, n.node8.keys[pos..], n.node8.keys[pos + 1 ..]);
            mem.copyForwards(?*Node, n.node8.children[pos..], n.node8.children[pos + 1 ..]);
            base.num_children -= 1;
        ...
        fn removeChild16(t: *Tree, _n: ?*Node, ref: *?*Node, l: ?**Node) Error!void {
            ...
            const base = n.baseNode();
            mem.copyForwards(u8, n.node16.keys[pos..], n.node16.keys[pos + 1 ..]);
            mem.copyForwards(?*Node, n.node16.children[pos..], n.node16.children[pos + 1 ..]);
            base.num_children -= 1;
            ...
            if (base.num_children == 7) {
                ...
                mem.copyForwards(?*Node, new_node.node8.children, n.node16.children[0..7]);
                t.deinitNodeBytes(n);
            }
        }
