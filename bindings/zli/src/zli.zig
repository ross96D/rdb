    ...
    next_arg: while (args.next()) |arg| {
        ...
        inline for (fields[0..field_count]) |field| {
            ...
            if (strings.eql(arg_name_app, arg_name_cli)) {
                ...
                const value = argx.parse_arg(field.type, arg) catch retry: {
                    const argValue = args.next().?;
                    break :retry argx.parse_value(field.type, arg, argValue);
                };
        ...
        if (@hasDecl(Args, "aliases")) {
            ...
            inline for (std.meta.fields(@TypeOf(aliases))) |field| {
                ...
                if (strings.eql(alias, arg_name_cli)) {
                    ...
                    const value = argx.parse_arg(field_type, arg) catch retry: {
                        const argValue = args.next().?;
                        break :retry argx.parse_value(field_type, arg, argValue);
                    };
                    @field(result, field.name) = value;
